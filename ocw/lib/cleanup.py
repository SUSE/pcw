...
def cleanup_k8s():
    for namespace in PCWConfig.get_namespaces_for('k8sclusters'):
        try:
            ...
            logger.debug("[{}] Run k8s cleanup for {}".format(namespace, ','.join(providers)))

            if 'ec2' in providers:
                EC2(namespace).cleanup_k8s_jobs()

        except Exception as e:
            logger.exception("[{}] k8s cleanup failed!".format(namespace))
            send_mail('{} on k8s cleanup in [{}]'.format(type(e).__name__, namespace), traceback.format_exc())
...
def init_cron():
    ...
    getScheduler().add_job(cleanup_k8s, trigger='interval', minutes=1, id='cleanup_k8s_all', misfire_grace_time=1800)
